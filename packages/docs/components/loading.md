...
```typescript
import { LzLoading } from "lz-element";
```

需要的时候通过以下方式调用

```typescript
LzLoading.service(options);
```

LoadingService 会返回一个 Loading 实例，可通过调用该实例的 close 方法来关闭它

```typescript
const loading = LzLoading.service(options);
nextTick(() => {
  loading.close();
});
```
...
| lz-loading-text       | 加载文案         | `string`  |
| lz-loading-spinner    | 加载图标         | `string`  |
| lz-loading-background | 遮罩背景色       | `string`  |
